...
** Last update Sun Nov 11 20:38:41 2012 maxime ginters
*/
...
void	check_expr(char *expr, t_base *base, char ops[7])
{
  int	i;
  int	found;
  int	j;
  ...
  while (expr[i] != '\0')
  {
    ...
    if (found == 0)
    {
      my_putstr("Error : invalid calcul\n");
      exit(1);
    }
    ++i;
  }
