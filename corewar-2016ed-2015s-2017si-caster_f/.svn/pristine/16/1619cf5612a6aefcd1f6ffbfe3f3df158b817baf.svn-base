/*
** cmd_ldi.c for commands in /home/fest/Epitech/CoreWar/core/commands
**
** Made by maxime ginters
** Login  <ginter_m@epitech.eu>
**
** Started on  Mon Dec 10 18:43:43 2012 maxime ginters
** Last update Thu Dec 13 18:27:17 2012 maxime ginters
*/
...
int	get_ldi_first_data(t_vm *vm, t_program *prog,
    unsigned char buffer[IND_SIZE], int param[4][2])
{
  ...
  if (param[0][0] == PARAM_REG)
  {
    ...
    tmp = getnbr_wsize(vm->memory, avance_mem(param[0][0] == PARAM_IND ?
      prog->pc : 0, getnbr_wsize(vm->memory, i, param[0][1]) % IDX_MOD), IND_SIZE);
  i = -1;
  tmp2 = IND_SIZE - 1;
...
int	get_ldi_second_data(t_vm *vm, t_program *prog, int *data,
    int param[4][2])
{
  char	type;
  int	tmp;
...
int	handle_ldi(t_vm *vm, t_program *prog, int index)
{
  ...
  parse_ldi_param(vm, prog, param);
  if (get_ldi_first_data(vm, prog, buffer, param) != 0 ||
      get_ldi_second_data(vm, prog, &i, param) != 0)
  {
  ...
  i = avance_mem(prog->pc, (i + avance_mem(param[0][0] == PARAM_DIR ? 0 :
          prog->pc,(getnbr_wsize(buffer, 0, IND_SIZE) % IDX_MOD))) % IDX_MOD);
