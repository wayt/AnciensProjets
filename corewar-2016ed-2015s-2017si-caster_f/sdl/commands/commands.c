...
** Last update dim. déc. 16 18:18:36 2012 maxime ginters
*/
...
int	handle_void(t_vm* vm, t_program* prog, int index)
{
  (void)vm;
  (void)prog;
  (void)index;
...
char	get_param_type(char oct, char index)
{
   char	val;
...
int	get_size_to_read(char oct, char index)
{
  char	type;
...
unsigned char	*mem_safecpy(unsigned char *dest, unsigned char *mem, int from,
    int size)
{
  int	i;
  int	src_i;
